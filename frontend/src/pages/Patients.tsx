...
export default function Patients() {
  ...
  const { data, loading, refreshing } = useApiResource(
    () => patientService.list({ page, limit, search: debouncedSearch || undefined, isActive: 'true' }),
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title="Patient registry" subtitle="Records are confidential and access is audited.">
        {refreshing && <p className="mb-3 text-xs font-medium muted">Refreshing records...</p>}
        {loading ? (
                    ...
                    <tr key={p.id} className="table-row-hover">
                      ...
                      <td className="table-cell whitespace-nowrap">
                        <div className="flex gap-3">
                          <Link to={`/patients/${p.id}`} className="link-primary">
                            View
                          </Link>
                          <Link to={`/patients/${p.id}/edit`} className="link-muted">
                            Edit
                          </Link>
                        </div>
                      </td>
